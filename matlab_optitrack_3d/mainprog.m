...
bots = struct('X',{0},'Y',{0},'Z',{0},'yaw',{0},'visible',{0},'name',robot_names,...
    'history',{ones(MOTION_HISTORY_SIZE,3)*-1},'histangle',{ones(MOTION_HISTORY_SIZE,1)*-1},...
    'hist_index',{1},'drawhistory',{ones(HISTORY_SIZE,3)*-1},'draw_hist_index',{1});
...
while 1
    ...
    if rem(frameCount,4) == 0
        plot_bots(fig, LINE_LEN, X_MAX, Y_MAX, Z_MAX, bots, waypoints, walls,...
    ...
    if USE_SERVER == 1       
        ...
        if send_launch == 1
            server_send_waypoints(waypoints);
            server_send_robots(bots);
            judp('SEND',4000,'192.168.1.255',int8(['GO ' int2str(size(waypoints,3)) ' ' int2str(run_number)]));
